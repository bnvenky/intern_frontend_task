...
const Candidates = () => {
  ...
  const filteredPositions = applyFilters(candidates);
  ...
  const handleAddCandidates = async (newCandidates) => {
    try {
      const response = await axios.post('http://localhost:8080/api/candidates', newCandidates, {
        headers: {
...
>
  ...
  {Object.values(filters).some((f) => f.length > 0) && (
    <span className="absolute top-0 right-0 bg-red-500 text-white text-xs rounded-full px-1">
            ...
            <>
              {view === 'kanban' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {currentPositions.map((candidate) => (
                    <CandidateCard key={candidate._id} candidate={candidate} />
                  ))}
                </div>
              ) : (
                <CandidateTable candidates={currentPositions} />
              )}
      ...
      {isModalOpen && (
        <AddCandidates
        isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={handleAddCandidates}
        />
      )}
